...
VALID_VISUAL_PROTO_MODES = ("original", "mean", "shrinkage_base_prior")
...
def refine_visual_prototypes(
    ...
    dynamic_lambda_max=0.95,
    tau=16.0,
):
    if mode not in VALID_VISUAL_PROTO_MODES:
        raise ValueError(f"Unknown Phase 2 visual prototype mode: {mode}")
    if mode in ("original", "mean") or base_visual_proto is None or base_visual_proto.numel() == 0:
    ...
    lambdas = _visual_shrinkage_lambdas(
        ...
        dynamic_lambda_max=dynamic_lambda_max,
    )
...
def _visual_shrinkage_lambdas(
    ...
    lambda_max = float(dynamic_lambda_max)
    default = torch.full(
    ...
    if not dynamic_lambda or support_features is None or support_labels is None or class_ids is None:
        return default.clamp(0.0, lambda_max)
    ...
    for local_idx, class_id in enumerate(class_ids):
        ...
        if count == 0:
            values.append(min(lambda_max, max(default_lambda, 0.5)))
            continue
        ...
        values.append(min(lambda_max, max(0.0, lambda_i)))
